...
import QUERY from './Parts.graphql';
...
class PartDropdown extends PureComponent<$FlowLintFix, State> {
...
const DefaultValueQuery = ({ value, onChange, ...props }: Object) => {
  ...
  if (!value) {
    return <PartDropdown {...props} value={undefined} onChange={handleOnChange} />;
  }

  if (Array.isArray(value) && typeof value[0] !== 'string') {
    return <PartDropdown {...props} value={value} onChange={handleOnChange} />;
  }
  ...
  return (
    ...
    >
      {({ data, loading }) => {
        const parts = noSelect ? [addnOption, ...(data.parts || [])] : data.parts;
        return (
          <PartDropdown
            {...props}
          ...
          <I18n ns="translations">
            {t => (
              <DefaultValueQuery
                ...
                placeholder={t('generic.selectPart')}
                {...props}
              />
            )}
          </I18n>
...
export const SelectPart = pure(DefaultOptionsQuery);
