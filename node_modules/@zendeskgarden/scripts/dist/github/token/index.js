...
const execute = async (spinner) => {
    let retVal = process.env.GITHUB_TOKEN;
    if (!retVal) {
        try {
            const token = await (0, execa_1.default)('git', ['config', '--get', 'github.token']);
            retVal = token.stdout.toString();
        }
        catch (error) {
            (0, utils_1.handleErrorMessage)(error, 'github-token', spinner);
            throw error;
        }
    }
    return retVal;
...
exports.default = (spinner) => {
    const command = new commander_1.Command('github-token');
    return command
